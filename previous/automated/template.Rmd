---
title: "`r {if (knitr::opts_chunk$get()$include) 'Template **solutions** using the automated approach via Make' else 'Template **questions** using the automated approach via Make'}`"
author: "Clement Lee"
date: "2021-03-17"
output:
  pdf_document: default
  html_docuement: default
classoption: a4paper
header-includes:
...
1. Edit this template as necessary.
2. Either go to the terminal tab within RStudio, or open a terminal if you are on Unix-like platforms.
3. Set the current directory to where this file is located, if this has not been done yet.
4. Type `make automated` in the terminal to generate all four documents in one go:
     i. Questions in pdf;
	 ii. Questions in html;
	 iii. Solutions in pdf;
	 iv. Solutions in html.
...
What are the main differences between this template and the "manual" template?

`r if (is_answer) {colorise("Solution:")}`

`r if (is_answer) {"
1. If you compare the first $\\texttt{R}$ code chunk i.e. the code between the pair of lines with triple backticks, the following line is not in the source code of this automated template:
"}`
...
`r if (is_answer) {"
1. (cont'd) This is because the option $\\texttt{include}$ is passed from the makefile, instead of being set manually.
2. While you can still generate a pdf or html manually by clicking the ***Knit*** button in RStudio, you will only get the solutions as $\\texttt{include}$ is default to $\\texttt{TRUE}$ if it's not supplied.
3. This way of parametrising the report also allows the title to be changed according to the $\\texttt{include}$ option. See the line beginning with $\\texttt{title:}$ at the top of the source code.
"}`
...
What if I want to generate another set of {questions, answers} $\times$ {pdf, html} using this automated approach?

`r if (is_answer) {colorise("Solution:")}`

`r if (is_answer) {"
1. You will need to create another Rmd of source code, then add a few lines in the makefile, similar to the ones for this file. 
2. As an example, we have made a template block in the makefile, beginning with the line $\\texttt{another: another.Rmd}$. This means you need to first create the file 'another.Rmd', then type $\\texttt{make another}$ in a terminal.
"}`
